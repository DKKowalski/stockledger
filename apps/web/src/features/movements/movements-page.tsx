...
export function MovementsPage() {
  ...
  return <PageState>
    ...
    <Panel title={`Movement ledger (${rows.length})`} className="spaced">
      ...
      {rows.length ? <div className="table-wrap"><table><thead><tr><th>Date</th><th>Item</th><th>Type</th><th>Place</th><th>Reference</th><th className="num">Qty</th><th className="num">Sale price</th><th className="num">Sale total</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{rows.map((movement) => <tr key={movement.id}><td>{calendarDate(movement.movementDate)}</td><td><b>{movement.item?.name ?? 'Deleted item'}</b><small>{movement.note}</small></td><td>{movementMeta[movement.type].label}</td><td>{movement.destination ? `${movement.location?.name ?? 'Unknown'} → ${movement.destination.name}` : movement.location?.name ?? 'Unknown'}</td><td>{movement.reference || '—'}</td><td className={`num ${movement.sign === 1 ? 'positive' : 'negative'}`}>{movement.sign === 1 ? '+' : '−'}{movement.quantity}</td><td className="num">{movement.type !== 'sale' ? '—' : movement.unitPriceCents == null ? 'Not recorded' : money(movement.unitPriceCents)}</td><td className="num">{movement.type !== 'sale' ? '—' : movement.saleTotalCents == null ? 'Not recorded' : money(movement.saleTotalCents)}</td><td className="num"><button className="icon-button" aria-label="Delete movement" disabled={busy} onClick={() => accessToken && void mutate(() => api.deleteMovement(accessToken, movement.id), 'The stock movement was removed.')}><Trash2 size={16} /></button></td></tr>)}</tbody></table></div> : <EmptyState text="No movements match this filter." />}
    </Panel>
  </PageState>;
}
