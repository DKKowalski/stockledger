import { Check, LogOut, Menu, X, type LucideIcon } from 'lucide-react';
...
export type NavigationItem = readonly [path: string, label: string, icon: LucideIcon];
    ...
    <header className="topbar">
      <div className="topbar-inner">
        ...
        <nav id="primary-navigation" className={`top-nav ${menuOpen ? 'open' : ''}`} aria-label="Main navigation">
          {navigation.map(([to, label, Icon]) => <NavLink key={to} to={to} end={to === '/'} onClick={() => setMenuOpen(false)} className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`}>
